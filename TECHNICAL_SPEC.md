...
```sql
CREATE TYPE log_tag AS ENUM ('progress', 'problem', 'solution', 'learning');
...
```json
{
  ...
  "technical_specs": {
    "voltage": "3.3V",
    "interface": ["WiFi 6", "BLE 5.0", "I2C", "SPI", "UART"],
    "package": "SMD"
  },
  "datasheet_url": "https://...",
  ...
  "suggestions": [
    {
      ...
      "viability_pct": 85,
      "bom": [
        { "component_name": "ESP32-C6", "quantity_required": 1 },
        { "component_name": "Sensor DHT22", "quantity_required": 1 }
      ],
      "resources": ["https://..."]
    }
  ]
...
```json
{
  "description": "estación meteorológica con WiFi y pantalla OLED",
  "inventory": [ ... ]
}
```
...
```json
{
  "title": "Estación Meteorológica WiFi + OLED",
  "description": "...",
  "difficulty": "intermediate",
  "bom": [
    {
...
```
Eres un experto en electrónica. Analiza la imagen y devuelve SOLO un JSON con:
{
  "name": "<nombre técnico>",
...
No incluyas explicaciones. Si no puedes identificar el componente con certeza,
devuelve confidence < 0.5.
```

### 4.4 Diseño del prompt — Inteligencia de proyectos

```
Eres un asistente de proyectos de electrónica. Dado este inventario:
{inventory_json}

Sugiere hasta 5 proyectos realizables ordenados por viabilidad (% de componentes
disponibles). Por cada proyecto devuelve JSON con:
{ title, description, difficulty, viability_pct, bom: [{component_name, quantity_required}], resources }
```
...
│   ├── projects/
│   │   ├── ProjectCard.tsx      # Tarjeta con estado y contadores
│   │   ├── LogTimeline.tsx      # Bitácora cronológica
│   │   ├── BOMTable.tsx         # Tabla BOM con estado de stock
